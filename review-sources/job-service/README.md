...
On restart, persisted `launch_pending` becomes `launch_unknown`; persisted `running` becomes `interrupted`. Neither is relaunched. Terminal state creates a durable, idempotent outbox record before any delivery attempt. `completionDelivery.enabled` reports whether this daemon has a configured delivery worker. With the worker enabled, nonterminal jobs report `awaiting_terminal`, and terminal outbox entries start at `ready`. With no completion configuration the job remains fully queryable and reports `enabled=false, state=disabled`; there is no Project or Task dependency.
...
### Completion presentation and consumption

Production uses `--completion-v2` to freeze action ID, exit code, execution observations and bounded stream summaries for Cutex's existing custom Job display. `exited` with exit code 0 renders as **Job completed**; absent exit code is not inferred as success. Protocol states remain machine-readable. Completion input uses `after_turn`; `read_output` and `query` do not acknowledge or suppress it. `delivered` is the receiving runtime's durable external-input acknowledgement, not proof that a human has read the message. Polling/retry attempts retain the same event ID.

On restart, legacy unattempted Disabled outbox entries normalize to Ready only when delivery is configured. Existing frozen bodies, event IDs, acknowledgements, and delivered receipts remain unchanged. The wire-version switch affects newly generated completions, not previously frozen events.
