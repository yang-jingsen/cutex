...
from real_provider_error_observer import observe,should_stop
...
def run_job(g):
    ...
    approvals=0;output_approvals=0;pending=list(rpc.events);rpc.events.clear();turns=set();active_deadlines={};completed_turns=set()
    model_deadline=time.monotonic()+600
    def save():
        (g['RUN']/'real-observations.json').write_text(json.dumps(observations,indent=2))
    def consume(msg):
        nonlocal approvals,output_approvals
        method=msg.get('method');p=msg.get('params',{})
        if method=='turn/started':
            turns.add(p['turn']['id']);observations['observedTurnCount']=len(turns)
            active_deadlines.setdefault(p['turn']['id'],time.monotonic()+300)
        ...
        if method=='error' or (method=='turn/completed' and p.get('turn',{}).get('error') is not None):
            observed=observe(msg)
            observations.setdefault('errors',[]).append(observed)
            save() # Before any stop/cleanup; never save original text/headers.
            if should_stop(msg,observed):
                observations['providerError']=observed
                raise RuntimeError('terminal or structural native error; no manual retry')
        ...
        if method=='turn/completed':
            active_deadlines.pop(p['turn']['id'],None)
            if p['turn'].get('status')=='failed':
                observations['failedTurnWithoutError']=p['turn'].get('error') is None
                save()
                raise RuntimeError('failed completed turn; no manual retry')
            assert p['turn'].get('status')=='completed','incomplete turn; no retry'
            completed_turns.add(p['turn']['id'])
            observations['completedTurnCount']=len(completed_turns)
        if 'id' in msg and method:
            ...
            if method=='mcpServer/elicitation/request':
                ...
                if p.get('serverName')=='cutex_job' and len(state['jobs'])==1 and len(turns)==2 and output_approvals==0:
                    jid,job=next(iter(state['jobs'].items()))
                    output_accepted=job['state']=='exited' and meta.get('tool_params')=={'jobId':jid,'stream':'stdout'}
                accepted=accepted or output_accepted
                rpc.send(json.dumps({'id':msg['id'],'result':{'action':'accept' if accepted else 'decline','content':{},'_meta':None}}).encode())
                if output_accepted:output_approvals+=1
                elif accepted:approvals+=1
            if not accepted:raise RuntimeError('unapproved native tool request; stopping')
        save()
    def remaining():
        return min([model_deadline,*active_deadlines.values()])-time.monotonic()
    def next_event():
        if remaining()<=0:raise TimeoutError('authorized model observation deadline')
        if pending:return pending.pop(0)
        try:return rpc.messages.get(timeout=min(remaining(),1))
        except queue.Empty:return None # Recheck authoritative Job state, not a model poll.
    def until_turn(ident):
        deadline=min(model_deadline,time.monotonic()+300)
        active_deadlines.setdefault(ident,deadline)
    ...
    while remaining()>0:
        state_path=g['RUN']/'job-state/state.json'
        state=json.loads(state_path.read_text()) if state_path.exists() else {'jobs':{}}
        if len(completed_turns)==2 and state['jobs'] and all(j.get('completionDelivery',{}).get('state')=='delivered' for j in state['jobs'].values()) and 'real-job-output' in '\n'.join(observations['messages']):break
        msg=next_event()
        if msg is not None:consume(msg)
