"""One bounded actual-provider smoke. No response fabrication or Core meta injection.
...
def run_job(g):
    ...
    def consume(msg):
        ...
        if method=='item/completed':
            ...
            if item.get('type')=='mcpToolCall':
                observations['toolCalls'].append({k:item.get(k) for k in ['server','tool','status','error']})
    ...
    first=rpc.call('turn/start',{'threadId':g['thread'],'input':[{'type':'text','text':'Private connectivity smoke. Reply exactly: TERRA_LOW_READY. Do not call any tools.'}]})
    until_turn(first['turn']['id'])
    assert 'TERRA_LOW_READY' in '\n'.join(observations['messages'])
    prompt='Private approved smoke. Use only cutex_job.submit once with exactly these arguments: '+json.dumps(expected)+'. Then end your reply with Submitted, without polling or other tools. When the Job completion arrives, use cutex_job.read_output once for stdout and briefly acknowledge the exact output. Do not read any other files, credentials or configuration. Do not use shell or exec tools directly.'
