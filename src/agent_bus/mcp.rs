...
pub fn tools() -> Value {
    let mut result = json!({"tools":[
        ...
        {"name":"send","description":"Send a message to a Cutex Agent using its exact ID and the selected delivery mode.","inputSchema":{"type":"object","properties":{"to":{"type":"string","description":"Exact durable Cutex Agent ID returned by query_managed."},"message":{"type":"string","description":"Include only context that changes the recipient’s actions or judgment. For delegation, state the task, constraints, and expected result; link longer references."},"external_message_id":{"type":"string","description":"Message identifier. Reuse it when retrying the same unchanged message."},"delivery_mode":{"type":"string","enum":["after_turn","soon","passive","interrupt"],"description":"Select when the message should be delivered."}},"required":["to","message","external_message_id","delivery_mode"],"additionalProperties":false}}
    ]});
    result["tools"]
        .as_array_mut()
        .unwrap()
