...
## Director-requested bounded plugin follow-up

Resolved the exact configured key `sample@debug` without executing a plugin or loader. GLM profile has `enabled=true`, but neither that profile nor shared native config has a `marketplaces` entry. The authoritative shared home `/home/senxiu/.cutex/codex-home` has no `plugins/cache/debug` directory (therefore no `debug/sample/<version>` installation), no `.tmp/marketplaces/debug`, and no `.agents/plugins/marketplace.json`. All four supported home-level marketplace locations under `/home/senxiu` are also absent. No asset source for this key was found through these configured/default locations. This is a configured enabled-but-uninstalled entry, not evidence of an operational plugin.

Exact old K `ef53716b7673ad14c24b977667334e31e66110d8` and new server `b8e9cc…` agree: `core-plugins/src/store.rs:26,131` resolves `plugins/cache/<marketplace>/<plugin>`; `loader.rs` initializes empty skills/MCP/apps/hooks and returns `plugin is not installed` when there is no active installation (old line869, new around875). `installed_marketplaces.rs:12–80` resolves configured local/default marketplace roots. No loader was run: it has install/synchronization paths, inappropriate for this read-only task.

Actual provided tools/skills/hooks: none established from installed assets; their intended contents are unknown, not guessed from “sample” or “debug”. Do not fetch/install it or silently mark it disabled. A future projection can preserve the explicit enabled key and visible missing-installation diagnostic without claiming tool availability; if Human requires the intended plugin functionality, the missing exact marketplace/asset source is the required input. This narrows the earlier plugin uncertainty: there is no evidence of a new-native loss of an installed capability at this shared path. Other project-specific marketplace overrides or historical installation state were not broadly scanned or asserted absent; the fixed store's active installation prerequisite remains absent now.

No product, runtime, auth or install changes. Authentication and custom-status decisions remain pending. The native owner's separate history result arrived independently: `artifacts/selected-k-history-r1/RESULT.md` SHA `64a1fd395dc9e46ee06bd2c0bec1a5ea6392824f680d3864567b9451e0601120` reports 32/34 histories with legacy InterAgentMessage typed rejection; that is a separate migration compatibility gate, not repaired here or counted as profile success.

## Independent auth candidate: actual path metadata preflight

Director authorized this read-only check while native `2eab060b191a0fe59e22b28785d02d76eafb7fc4` is under independent review, **not yet accepted**. Read its `artifacts/independent-auth-file-r1/INTERFACE.md` and `RESULT.md`. Proposed launch-only spelling is `--auth-file ABS_PATH`, File/Linux only; remote attach rejects this option. No product enabling or pins changed here.

Used lstat only, no auth content/open/read: all three exact `profiles/<ID>/auth.json` files from the table above are UID1000, regular0600, single-link, not symlinks. `/` and `/home` are root0755; `/home/senxiu` UID1000/0700. However `/home/senxiu/.cutex`, its `profiles` directory, and each of the three immediate profile directories are UID1000/**0775**, not symlinks. Thus all three paths fail the candidate's no-group-writable ancestor rule and the immediate-parent0700 rule. This is a metadata prediction, not an executed candidate launch/security test; no ownership or permissions changed.

Current materialization source `src/profiles/materialize.rs:39–44` selects those per-profile auth paths. The future projection must not paper over this by weakening native checks. Real-use preparation needs explicit authority either for an exact-directory permissions change after assessing existing writers, or a separately reviewed private custody location and refresh ownership plan. Neither was performed or selected. No broad chmod, symlink, per-launch shared-file replacement, or silent token-copy/sync-back proposal is authorized by this preflight.

Planned wiring once accepted: new typed review version binds selected profile/account identity and canonical independent path plus safe parent custody; direct owner launch adds the path-only option before auth/config loading and File mode explicitly. Stock same-owner remote attach **does not** receive auth-file; it attaches to the already authenticated owner. Keep old mode's home/path and receipt interpretation unchanged. Legitimate native atomic refresh changes file inode, so new review validation must not reuse the old permanent-inode equality rule: revalidate protected parent/path and same account semantics, while refusing unsafe/replaced account material. No token-value hash enters review. Bootstrap, explicit restart and pre-execution CAS must all use the same frozen selection. GLM still needs its separate fixed provider bearer-materialization path; selecting an auth file does not automatically override provider env-key semantics.

The auth interface can resolve the original shared-home architectural gap if accepted; current on-disk permissions are a distinct deployment preparation issue. Custom status preservation remains pending. No new model, process, VM, credential copy, install, build or cache activity occurred in this follow-up.
